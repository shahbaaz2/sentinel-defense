...
Apply MissionNet migrations: `make migrate-missionnet` (equivalent to
`alembic -c infrastructure/migrations/missionnet/alembic.ini upgrade head`). To create a new one after
changing `apps/missionnet/models.py`:
```bash
